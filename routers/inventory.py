...
def get_stock_summary(
    ...
    if search:
        ...
        for kw in keywords:
            like = f"%{kw}%"
            where.append("(v.item_code LIKE %s OR v.item_name LIKE %s OR v.item_print_name LIKE %s OR v.category LIKE %s)")
            params.extend([like, like, like, like])
    if needs_reorder is True:
        where.append("v.needs_reorder = 1")
    elif needs_reorder is False:
        where.append("v.needs_reorder = 0")
    if category:
    ...
    if tag_ids:
        ...
        if ids:
            ...
            where.append(f"v.item_code IN (SELECT item_code FROM item_tag_map WHERE tag_id IN ({placeholders}) AND company_code = %s)")
            params.extend(ids)
